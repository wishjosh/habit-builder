import {CATEGORIES,today,dateObj,addDays,weekStart,monthDays,datesBetween,uid,clone,freshState,habitAt,entriesFor,cardsFor,markDone,undoDone,starSummary,redeem,cancelRedemption,saveHabit,frequencyLabel,touch,validateState,taskLabel,groupByCategory} from './engine.js';
...
function renderToday(){
  ...
  ${todayGroups(cards,childId,date,card=>habitCard(card,rest),category=>btn('add-habit',`${icon('plus')}할 일 추가`,'text-btn',`data-category="${category}"`))}${!cards.length?empty('이날은 정해 둔 할 일이 없어요.')+btn('add-habit',`${icon('plus')}할 일 추가`,'secondary'):''}
...
function renderMaterial(category){
  ...
  const required=['reading','math'].includes(category);field.required=required;
  dialog.querySelector('#material-label').textContent=category==='reading'?'책 제목':category==='math'?'교재 이름':'책·교재·준비물 이름 (선택)';
  field.placeholder=category==='reading'?'예: 톰 소여의 모험':category==='math'?'예: 눈높이 수학':'해당하는 경우 적어 주세요';
