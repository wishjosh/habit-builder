import {CATEGORIES,today,dateObj,addDays,weekStart,monthDays,datesBetween,uid,clone,freshState,habitAt,entriesFor,cardsFor,markDone,undoDone,starSummary,redeem,cancelRedemption,saveHabit,frequencyLabel,touch,validateState} from './engine.js';
...
function renderToday(){
  ...
  <div class="habit-grid">${cards.map(card=>habitCard(card,rest)).join('')}</div>${!cards.length?empty('이날은 정해 둔 활동이 없어요.<br>부모 메뉴에서 우리 가족의 약속을 더해 보세요.'):''}
...
function habitCard({habit:h,config:v,entry,flexible,count,target,goalReached},rest){
  const cat=CATEGORIES[v.category],disabled=!entry&&(rest||goalReached);
  return `<article class="habit-card ${entry?'done':''}"><div class="card-top"><span class="activity-icon ${cat.color}">${icon(cat.icon)}</span><span class="reward-tag">${icon('star')}${entry?entry.points:v.points}</span></div><h3>${esc(entry?.snapshot.title||v.title)}</h3><p>${esc(entry?.snapshot.detail||v.detail)}</p><div class="card-footer"><span class="frequency">${frequencyLabel(v)}${flexible?`<br>${count} / ${target}번 실천`:''}</span>${btn(entry?'undo':'done',`${icon(entry||goalReached?'check':'plus')}${entry?'했어요!':goalReached?'약속 달성':rest?'쉬어 가요':'다 했어요'}`,`done-btn ${goalReached&&!entry?'goal':''}`,`data-id="${esc(h.id)}" aria-label="${esc(v.title)} ${entry?'완료 취소':'완료'}" aria-pressed="${!!entry}" ${disabled?'disabled':''}`)}</div></article>`;
}
function timeline(entries,showDate=true){return entries.map(e=>`<div class="timeline-item"><span class="activity-icon ${CATEGORIES[e.snapshot.category]?.color||'mint'}">${icon(CATEGORIES[e.snapshot.category]?.icon||'leaf')}</span><div class="grow"><strong>${esc(e.snapshot.title)}</strong><p>${showDate?`${Number(e.date.slice(5,7))}월 ${Number(e.date.slice(8))}일 · `:''}${esc(e.snapshot.detail)}</p></div><span class="reward-tag">${icon('star')}${e.points}</span></div>`).join('');}
function renderRecords(){
  ...
  if(recordTab==='week'){
    ...
    body=`<div class="activity-history">${habits.map(h=>{const rows=all.filter(e=>e.habitId===h.id),v=habitAt(h,today())||h.versions.at(-1),cat=CATEGORIES[v.category];return `<article class="activity-summary"><span class="activity-icon ${cat.color}">${icon(cat.icon)}</span><div class="grow"><h3>${esc(v.title)}${h.archivedFrom?' <small>지난 활동</small>':''}</h3><p>${rows.length?`최근 실천 ${rows.map(e=>e.date).sort().at(-1)}`:'첫 발자국을 기다려요'}</p></div><strong>${rows.length}<small>번</small></strong></article>`;}).join('')}</div>`;
  }
  return `${heading(`${esc(child().name)}의 발자국`)}<p class="date-label">작은 실천이 모여 나만의 이야기가 되어요.</p><div class="tabbar" aria-label="기록 보기">${[['week','이번 주'],['month','월간 달력'],['activity','활동별']].map(([id,name])=>btn('record-tab',name,recordTab===id?'active':'',`data-tab="${id}" aria-pressed="${recordTab===id}"`)).join('')}</div><div class="print-only">습관 형성 시스템 · ${esc(child().name)}</div>${body}`;
}
...
function renderParent(){
  ...
  return `${heading('우리 가족의 약속 만들기')}<div class="banner">예시 활동으로 시작했어요. 아이와 이야기하며 이름, 분량, 요일을 우리 가족에게 맞춰 주세요.</div><section class="panel"><h2>함께하는 아이들</h2>${state.children.map(c=>`<div class="settings-item">${avatar(c.avatar)}<div class="grow"><strong>${esc(c.name)}</strong><p>${c.style==='together'?'부모와 함께 기록해요':'스스로 확인하고 기록해요'}</p></div>${btn('edit-child','바꾸기','text-btn',`data-id="${esc(c.id)}"`)}</div>`).join('')}</section><section class="panel"><div class="section-head" style="margin-top:0"><h2>${esc(child().name)}의 활동</h2>${btn('add-habit',`${icon('plus')}활동 추가`,'secondary')}</div>${habits.length?habits.map(h=>{const v=habitAt(h,today())||h.versions.at(-1),cat=CATEGORIES[v.category];return `<div class="settings-item"><span class="activity-icon ${cat.color}">${icon(cat.icon)}</span><div class="grow"><strong>${esc(v.title)}</strong><p>${frequencyLabel(v)} · 별 ${v.points}개<br>${esc(v.detail)}</p></div>${btn('edit-habit','수정','text-btn',`data-id="${esc(h.id)}"`)}${btn('archive-habit','그만하기','text-btn',`data-id="${esc(h.id)}"`)}</div>`;}).join(''):empty('아이와 함께 첫 활동을 정해 보세요.')}<p>활동을 바꾸거나 그만해도 이미 남긴 기록과 별은 보관해요.</p></section><section class="panel"><div class="section-head" style="margin-top:0"><h2>우리 가족의 선물</h2>${btn('add-reward',`${icon('plus')}추가`,'text-btn')}</div>${state.rewards.map(r=>`<div class="settings-item"><span class="activity-icon yellow">${icon(r.icon||'gift')}</span><div class="grow"><strong>${esc(r.title)}</strong><p>별 ${r.cost}개</p></div>${btn('edit-reward','수정','text-btn',`data-id="${esc(r.id)}"`)}</div>`).join('')}${state.redemptions.some(r=>!r.cancelledAt)?`<details class="settings-section" style="margin-top:20px"><summary>선물 교환 내역 관리</summary>${state.redemptions.filter(r=>!r.cancelledAt).slice().reverse().map(r=>`<div class="settings-item"><div class="grow"><strong>${esc(state.children.find(c=>c.id===r.childId)?.name)} · ${esc(r.title)}</strong><p>별 ${r.cost}개 · ${fmt(today(new Date(r.at)))}</p></div>${btn('cancel-reward','교환 취소','text-btn',`data-id="${esc(r.id)}"`)}</div>`).join('')}</details>`:''}</section>
...
function habitEditor(id){
  const h=state.habits.find(h=>h.id===id),v=h?(habitAt(h,today())||h.versions.at(-1)):{title:'',detail:'',category:'life',frequency:'daily',days:[1,2,3,4,5],target:3,points:1,dueDate:today()};
  modal(h?'활동 바꾸기':'새로운 작은 약속',`<form data-form="habit" data-id="${esc(id||'')}"><label class="field">활동 이름<input name="title" value="${esc(v.title)}" placeholder="예: 책과 만나는 시간" maxlength="60" required></label><label class="field">어디까지 하면 완료인가요?<input name="detail" value="${esc(v.detail)}" placeholder="예: 책을 15분 읽어요" maxlength="160"><small>아이도 알 수 있는 짧고 구체적인 말로 적어 주세요.</small></label><div class="form-grid"><label class="field">활동 종류<select name="category">${Object.entries(CATEGORIES).map(([id,c])=>`<option value="${id}" ${id===v.category?'selected':''}>${c.label}</option>`).join('')}</select></label><label class="field">언제 할까요?<select name="frequency">${[['daily','매일'],['weekdays','요일을 골라서'],['weekly','일주일에 몇 번'],['monthly','한 달에 몇 번'],['once','정한 날짜에 한 번']].map(([id,label])=>`<option value="${id}" ${id===v.frequency?'selected':''}>${label}</option>`).join('')}</select></label></div><div id="frequency-extra"></div><label class="field">완료하면 받는 별<input type="number" name="points" min="0" max="20" value="${v.points}" required><small>별을 사용하지 않을 활동은 0으로 정해 주세요.</small></label><p class="form-error" role="alert"></p><div class="dialog-actions">${btn('close','돌아가기')}<button class="primary" type="submit">약속 저장</button></div></form>`);
  renderFrequency(v.frequency,v);
}
...
document.addEventListener('click',event=>{
  ...
  try{
    ...
    if(action==='add-habit'||action==='edit-habit'){habitEditor(id);return;}
...
document.addEventListener('submit',async event=>{
  ...
  try{
    if(type==='habit'){const values=Object.fromEntries(data);values.days=data.getAll('days').map(Number);if(commit(s=>saveHabit(s,childId,id,values),'우리의 약속을 저장했어요.'))dialog.close();}
