...
import {weeklyAt,weeklyLabel,saveWeeklyPlan,saveDailyPlan,removeDailyPlan,recentMaterials,activeCategories,categoryInfo,saveCategory,deleteCategory,categoryHabits,today,dateObj,addDays,weekStart,monthDays,datesBetween,uid,clone,freshState,habitAt,entriesFor,cardsFor,markDone,undoDone,starSummary,redeem,cancelRedemption,saveHabit,frequencyLabel,touch,validateState,taskLabel,groupByCategory} from './engine.js';
...
function confirmModal(title,body,action,label,attrs=''){const shared=action==='archive-confirm'||action==='daily-remove-confirm';modal(title,`<p>${body}</p>${shared?parentCheck()+'<p class="form-error" role="alert"></p>':''}`,btn('close','돌아가기')+btn(action,shared?'함께 확인하고 '+label:label,'primary',attrs));}
