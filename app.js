...
import {ZODIAC_AVATARS,weeklyAt,weeklyLabel,saveWeeklyPlan,saveDailyPlan,removeDailyPlan,recentMaterials,historyHabits,activeCategories,categoryInfo,saveCategory,moveCategory,deleteCategory,categoryHabits,today,dateObj,addDays,weekStart,monthDays,datesBetween,uid,clone,freshState,habitAt,entriesFor,cardsFor,markDone,undoDone,starSummary,redeem,removeReward,cancelRedemption,saveHabit,frequencyLabel,touch,validateState,taskLabel,groupByCategory} from './engine.js';
...
function confirmModal(title,body,action,label,attrs=''){const shared=['archive-confirm','daily-remove-confirm','weekly-remove-confirm','remove-reward-confirm'].includes(action);modal(title,`<p>${body}</p>${shared?parentCheck()+'<p class="form-error" role="alert"></p>':''}`,btn('close','돌아가기')+btn(action,shared?'함께 확인하고 '+label:label,'primary',attrs));}
