...
function taskRemovalDialog(id,targetChild=childId){
  ...
  const v=habitAt(h,today())||h.versions.at(-1),records=Object.values(state.entries).filter(e=>e.habitId===id),points=records.reduce((sum,e)=>sum+e.points,0),balance=starSummary(state,h.childId).balance,active=!h.archivedFrom||h.archivedFrom>today();
